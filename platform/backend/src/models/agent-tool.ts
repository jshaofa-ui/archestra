...
class AgentToolModel {
  ...
  /**
   * NULL out the mcpServerId reference instead of deleting agent-tool assignments.
   * This preserves the agent-tool assignment when an MCP server credential is deleted,
   * allowing the tool to remain assigned to the agent (just without a credential source).
   * Related: archestra-ai/archestra#4423
   */
  static async nullifyMcpServerId(mcpServerId: string): Promise<number> {
    const result = await db
      .update(schema.agentToolsTable)
      .set({
        mcpServerId: null,
